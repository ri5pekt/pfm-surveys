    ...
    <table v-else class="websites-table">
      ...
      <tbody>
        <tr v-for="site in sitesStore.sites" :key="site.id">
          ...
          <td><code class="site-id">{{ site.site_id }}</code></td>
          <td>
            <span v-if="site.allow_any_domain" class="text-warning">🔓 Any domain (insecure)</span>
            <span v-else-if="!site.domains || site.domains.length === 0" class="text-danger">⚠️ No domains (blocked)</span>
        ...
        <div class="site-details">
          ...
          <div class="detail-item embed-section">
            ...
            <div class="embed-code-container">
              ...
              <button @click="copyToClipboard(getEmbedScript(viewingSite.site_id))" class="btn-copy-embed">
                Copy Script
              </button>
            </div>
            <div class="embed-note">
              <strong>📍 Current API URL:</strong> {{ apiUrl }}<br>
              <span v-if="apiUrl.includes('ngrok')">⚠️ This ngrok URL will change when you restart. For production, deploy to a permanent server.</span>
            </div>
          </div>
          ...
          <div class="detail-item">
            ...
            <div class="detail-value">
              <span v-if="viewingSite.allow_any_domain" class="text-warning">🔓 Any domain (insecure - dev/testing only)</span>
              <span v-else-if="!viewingSite.domains || viewingSite.domains.length === 0" class="text-danger">⚠️ No domains configured (all requests blocked)</span>
