<template>
    <div class="page-autocomplete" ref="containerRef">
        <input
            ref="inputRef"
            v-model="inputValue"
            @input="onInput"
            ...
            type="text"
            :name="`page-filter-${Math.random().toString(36).slice(2)}`"
...
async function fetchSuggestions() {
    ...
    let endpoint = `${API_BASE}/api/autocomplete/pages`;
    ...
    try {
        ...
        if (!response.ok) {
            console.error("Failed to fetch page suggestions:", response.statusText);
            suggestions.value = [];
            return;
        }
